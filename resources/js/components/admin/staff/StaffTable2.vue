    ...
    >
      <template v-slot:top>
        ...
        <v-toolbar flat color="white">
          <v-spacer></v-spacer>
          <v-switch v-model="singleExpand" label="Expandir solo uno" class="mt-2"></v-switch>
        </v-toolbar>
      </template>
      ...
      <template v-slot:expanded-item="{ headers, item }">
        <td :colspan="headers.length" >
          <strong>{{ item.firstname }} {{ item.firstlastname }} {{ item.secondlastname }}</strong>
          reside en
          <strong>{{ item.address }}</strong> ubicado en el distrito
          <strong>{{ item.district }}</strong>
          de la ciudad de
          <strong>{{ item.city }}</strong> ,
          su numero de telefono para contactos es
          <strong>{{ item.phone }}</strong> y su email es
