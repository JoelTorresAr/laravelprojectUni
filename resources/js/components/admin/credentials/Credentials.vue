    ...
    >
      ...
      <template v-slot:item.roles="{ item }">
        <v-chip
          ...
          dark
          v-for="(rol, index) in item.roles" :key="index"
        >{{ rol.name }}</v-chip>
      </template>
      ...
      <template v-slot:item.actions="{ item }">
        ...
        <v-btn color="error" fab x-small dark @click>
        ...
        <v-dialog v-model="dialogStaff" scrollable max-width="300px">
          <v-card>
            ...
            <v-card-text style="height: 300px;">
              <form @submit.prevent="save">
                <v-radio-group v-model="form.staff_id" column>
                  <v-radio
                    v-for="(item, index) in staff"
                    :key="index"
                    :value="item.id"
                    :label="nameStaff(item)"
                  ></v-radio>
                </v-radio-group>
              </form>
            </v-card-text>
...
export default {
  data: () => ({
    ...
    headers: [
      ...
      { text: "Nombre de acceso", value: "username" },
      { text: "Descripcion", value: "description" },
      { text: "Fecha de creación", value: "created_at" },
      { text: "Fecha de modificación", value: "updated_at" },
      { text: "Estado", value: "staff_id" },
      { text: "Roles", value: "roles" },
      { text: "Usuario", value: "firstname" },
  ...
  methods: {
    ...
    nameStaff(item) {
      ...
      if (item.staff !== null) {
        name =
          item.staff.firstname + " " + item.staff.firstlastname + " " + item.staff.secondlastname;
      }
      return name;
    },
    formatedTime(t) {
