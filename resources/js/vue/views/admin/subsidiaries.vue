...
export default {
  ...
  methods: {
    initialize() {
      axios.get("/api/subsidiaries/").then(({ data }) => {
    ...
    save() {
      if (this.editedIndex === -1) {
        ...
        let url = "/api/subsidiaries/update/" + this.itemSelectedId;
        this.editedItem
          .put(url)
          .then(({ data }) => {
            if (data.status == "200") {
    ...
    deleteItem(item) {
      let url = "/api/subsidiaries/destroy/" + item.id;
      const index = this.subsidiaries.indexOf(item);
