  ...
  methods: {
    ...
    save() {
      if (this.editedIndex === -1) {
        ...
        let url = "/api/prooftypes/update/" + this.itemSelectedId;
        this.editedItem
          .put(url)
          .then(({ data }) => {
            if (data.status == "200") {
    ...
    deleteItem(item) {
      Swal.fire({
        ...
        if (result.value) {
          let url = "/api/prooftypes/destroy/" + item.id;
          const index = this.prooftypes.indexOf(item);
          axios
            .delete(url)
            .then(({ data }) => {
              if (data.status == "200") {
                this.prooftypes.splice(index, 1);
